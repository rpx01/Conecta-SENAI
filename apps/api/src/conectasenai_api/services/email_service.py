...
from conectasenai_common import (
    Address,
    RateLimiter,
    build_turma_context,
    build_user_context,
...
if TYPE_CHECKING:
    from conectasenai_api.models.treinamento import TurmaTreinamento
...
def send_email(
    ...
    if text:
        params["text"] = text
    if cc:
        params["cc"] = normalize_addresses(cc)
    if bcc:
        params["bcc"] = normalize_addresses(bcc)
...
def enviar_notificacao_planejamento(
    ...
    from conectasenai_api.models import EmailSecretaria  # import lazy to evitar ciclo
...
def listar_emails_secretaria() -> List[str]:
    ...
    from conectasenai_api.models.secretaria_treinamentos import (
...
def notificar_atualizacao_turma(
    ...
    *,
    ...
    if old_horario:
        ...
        hora_ini = parse_time(partes[0]) or turma_ctx.horario_inicio
        hora_fim = parse_time(partes[1]) if len(partes) > 1 else None
    ...
    if instrutor_antigo_obj and not getattr(
        ...
        try:  # tenta carregar o instrutor a partir do ID
            from conectasenai_api.models.instrutor import Instrutor  # lazy import
