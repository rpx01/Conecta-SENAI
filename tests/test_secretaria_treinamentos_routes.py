...
def test_crud_contatos_secretaria(client, app):
    ...
    # Listagem
    resp = client.get("/api/treinamentos", headers=headers)
    ...
    # Exclusão
    resp = client.delete(f"/api/treinamentos/{contato_id}", headers=headers)
    assert resp.status_code == 200
...
def test_criar_contato_email_duplicado(client, app):
    ...
    r1 = client.post("/api/treinamentos", json=payload, headers=headers)
    assert r1.status_code == 201
    r2 = client.post("/api/treinamentos", json=payload, headers=headers)
